...
void sanity_checks()
{
	...
	if (test.width() != image.width()) {
		fprintf(stderr, "Sanity check failed: your Qt installation is broken (test width %d != image width %d) You can try to fix it by export QT_PLUGIN_PATH=/usr/local/Cellar/qt/4.8.7_2/plugins\n", test.width(), image.width());
		exit(1);
	}
}
